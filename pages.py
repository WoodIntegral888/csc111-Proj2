...
class StarterPage(tk.Frame):
    ...
    def load_started_btn(self, root):
        ...
        button.bind("<Enter>", lambda e: root.switch_btn_hover(button, btn_hover, btn_static, True))
        button.bind("<Leave>", lambda e: root.switch_btn_hover(button, btn_hover, btn_static, False))


class QuestionsPage(tk.Frame):
    genres_list = ['action', 'comedy', 'documentary', 'drama', 'horror', 'mystery', 'romance', 'science ficition']
    genre_variables = []
    favMovieAnswerVar = ''
    ...
    def load_fav_movie_question(self):
        favMovieQuestion = tk.Label(self,
        ...
        favMovieAnswerBox = tk.Entry(self,
                                     bg="#85CFFF",
                                     font=("Futura-Bold", 20),
                                     fg="#3C1D53",
    ...
    def load_fav_genre_question(self):
        favGenreQuestion = tk.Label(self,
        ...
        for j in range(len(self.genres_list)):
            genreOption = tk.Checkbutton(self,
    ...
    def check_submit(self, root):
        if not self.is_valid_movie(root):
        ...
        elif self.is_valid_movie(root):
            root.switch_frame("MovieConfirmPage")

    def is_valid_movie(self, root) -> bool:
        if self.favMovieAnswerVar.get() == "":
            return False
        elif len(difflib.get_close_matches(self.favMovieAnswerVar.get(), root.movies, n=1)) == 0:
            return False
        else:
            root.chosen_movie = self.favMovieAnswerVar.get()
            return True
...
class MovieConfirmPage(tk.Frame):
    ...
    def load_options(self, root, button):
        ...
        print("CHOSEN MOVIE:" + root.chosen_movie)
        search_results = difflib.get_close_matches(root.chosen_movie, root.movies, n=5)
        ...
        for result in search_results:
            ...
            self.load_interact_option(root, imgStatic_file, imgHover_file, f"{result.upper()}")



    def load_interact_option(self, root, static_img_file, hover_img_file, movie_name: str):
        ...
        resultDisplay.bind("<Button-1>", lambda e: self.set_selected_movie(root, movie_name))
        resultDisplay.bind("<Enter>", lambda e: root.switch_btn_hover(resultDisplay, hoverImg, staticImg, True))
        resultDisplay.bind("<Leave>", lambda e: root.switch_btn_hover(resultDisplay, hoverImg, staticImg, False))
